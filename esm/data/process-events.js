...
function eventChangesWarnings(fiber, changes) {
    ...
    if (fiber.type === ElementTypeProvider && changes?.props) {
        const shallowValueChange = changes.props.find(change => change.name === "value" && isShallowEqual(change));
...
export function processEvents(newEvents, allEvents, { linkedEvents, commitById, fiberById, fiberTypeDefById, fibersByTypeId, fibersByProviderId, leakedFibers, parentTree, parentTreeIncludeUnmounted, ownerTree, ownerTreeIncludeUnmounted, }) {
    const newEventsByCommitId = new Map();
    let mountCount = 0;
    let unmountCount = 0;
    let updateCount = 0;
    ...
    const linkEvent = (linkedEvent) => {
        ...
        const trigger = ("trigger" in event &&
    ...
    const normChanges = (fiber, changes) => {
        ...
        const normalizedChanges = {
            ...
            context: changes.context?.map(change => {
                const linkedEvent = linkedEvents.get(allEvents[change.valueChangedEventId]);
    ...
    for (const event of newEvents) {
        ...
        switch (event.op) {
            case "fiber-type-def": {
                fiberTypeDefById.set(event.typeId, {
                    ...
                    hooks: event.definition.hooks.map((hook, index) => ({
                        index,
                        ...hook,
                        context: typeof hook.context === "number"
            ...
            case "mount": {
                ...
                fiber = {
                    ...event.fiber,
                    typeDef,
                    displayName: event.fiber.displayName ||
        ...
        fiber = {
            ...fiber,
            events: fiber.events.concat(linkEvent({
                target: "fiber",
