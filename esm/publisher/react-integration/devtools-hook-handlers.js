...
import { ElementTypeClass, ElementTypeFunction, ElementTypeMemo, ElementTypeForwardRef, ElementTypeProvider, ElementTypeConsumer, ElementTypeHostRoot, ElementTypeHostComponent, ElementTypeHostText, } from "../../common/constants";
...
export function createReactDevtoolsHookHandlers({ ReactTypeOfWork, 
// ReactPriorityLevels,
getFiberTypeId, getOrGenerateFiberId, getFiberIdThrows, getFiberIdUnsafe, getFiberOwnerId, getFiberById, removeFiber, getElementTypeForFiber, getDisplayNameForFiber, setRootPseudoKey, didFiberRender, removeRootPseudoKey, shouldFilterFiber, }, { getDispatchHookIndex, getFiberTypeHookInfo, flushDispatchCalls, }, recordEvent) {
    ...
    //   NoPriority,
    // } = ReactPriorityLevels;
    const idToOwnerId = new Map();
    const commitUpdatedFiberId = new Map();
    const commitTriggeredFiberId = new Set();
    const commitClassFiberUpdateCalls = new Map();
    const commitFiberUpdateCalls = new Map();
    ...
    const unmountedFiberIds = new Set();
    const unmountedFiberIdsByOwnerId = new Map();
    const unmountedFiberIdBeforeSiblingId = new Map();
    const unmountedFiberIdForParentId = new Map();
    ...
    function getComponentChange(prevFiber, nextFiber) {
        ...
        if (type === ElementTypeHostText) {
            ...
            return {
                props: getPropsChanges({ "#text": prevFiber.memoizedProps }, { "#text": nextFiber.memoizedProps }),
            };
        }
        ...
        const data = {
            ...
            ...(isElementTypeClass
                ? {
                    ...
                    state: getStateChanges(prevFiber.memoizedState, nextFiber.memoizedState, prevFiber),
                }
                : {
                    // Functional component
                    context: getFunctionContextChanges(nextFiber),
                    state: getStateHooksChanges(prevFiber.memoizedState, nextFiber.memoizedState),
    ...
    function getMemoHookChanges(fiber) {
        const hookMemoIndecies = recordedTypeDef.get(getFiberTypeId(fiber.type, fiber.tag))
            ?.hookMemoIndecies || [];
        const changes = [];
    ...
    function getFiberContexts(fiber, fiberType, fiberHooks) {
        ...
        const hookContexts = fiberHooks.reduce((contexts, hook) => hook.context != null ? contexts.add(hook.context) : contexts, new Set());
    ...
    function recordFiberTypeDefIfNeeded(fiber, typeId, fiberType) {
        ...
        const hookContextIndecies = new Map();
        ...
        for (const hook of hooks) {
            ...
            if (hook.context) {
                ...
                if (hookContext === undefined) {
                    hookContextIndecies.set(hook.context, (hookContext = hookContextIndecies.size));
                }
            }
        ...
        if (fiberType === ElementTypeClass) {
            ...
            if (updater.enqueueForceUpdate.patched !== PATCHED_UPDATER) {
                ...
                Object.defineProperties(updater, {
                    enqueueForceUpdate: {
                        value: Object.assign(function (inst, callback) {
    ...
    function locFromDebugSource({ fileName, lineNumber, columnNumber, }) {
        return typeof fileName === "string" &&
            typeof lineNumber === "number" &&
            lineNumber > 0
            ? `${fileName}:${lineNumber}${typeof columnNumber === "number" && columnNumber > 0
                ? ":" + columnNumber
                : ""}`
            : null;
    }
    function recordMount(fiber, parentFiber) {
        ...
        if (isRoot) {
            ...
            const { displayName, hocDisplayNames } = separateDisplayNameAndHOCs(getDisplayNameForFiber(fiber), elementType);
            ...
            props = Object.keys(elementType !== ElementTypeHostText
                ? fiber.memoizedProps
    ...
    function recordUpdate(fiber) {
        ...
        const triggerEventId = fiber.memoizedProps !== alternate.memoizedProps
        ...
        if (didFiberRender(alternate, fiber)) {
            ...
            if (changes !== null &&
                ...
                if (valueChange !== undefined) {
                    const contextInfo = commitContext.get(fiber.type._context || fiber.type.context);
            ...
            recordEvent({
                ...
                changes: {
                    ...
                    state: getStateChanges(fiber.memoizedState, alternate.memoizedState, fiber),
                },
                trigger: triggerEventId,
            });
        }
        ...
        else if (commitUpdatedFiberId.has(ownerId) &&
    ...
    function handleCommitFiberRoot(root /*, priorityLevel?: number*/) {
        const { current } = root;
