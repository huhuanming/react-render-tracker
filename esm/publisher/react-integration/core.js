...
import { CONCURRENT_MODE_NUMBER, CONCURRENT_MODE_SYMBOL_STRING, CONTEXT_NUMBER, CONTEXT_SYMBOL_STRING, DEPRECATED_ASYNC_MODE_SYMBOL_STRING, PROFILER_NUMBER, PROFILER_SYMBOL_STRING, PROVIDER_NUMBER, PROVIDER_SYMBOL_STRING, STRICT_MODE_NUMBER, STRICT_MODE_SYMBOL_STRING, } from "./utils/constants.js";
import { ElementTypeClass, ElementTypeFunction, ElementTypeForwardRef, ElementTypeMemo, ElementTypeProvider, ElementTypeConsumer, ElementTypeHostRoot, ElementTypeHostComponent, ElementTypeHostText, ElementTypeHostPortal, ElementTypeSuspense, ElementTypeSuspenseList, ElementTypeProfiler, ElementTypeOtherOrUnknown, TrackingObjectFiber, TrackingObjectStateNode, TrackingObjectAlternate, TrackingObjectHook, } from "../../common/constants";
import { createUnmountedFiberLeakDetectionApi } from "./unmounted-fiber-leak-detector";
export function createIntegrationCore(renderer, recordEvent) {
    // Newer versions of the reconciler package also specific reconciler version.
    // If that version number is present, use it.
    // Third party renderer versions may not match the reconciler version,
    // and the latter is what's important in terms of tags and symbols.
    const version = renderer.reconcilerVersion || renderer.version || "";
    const { getDisplayNameForFiber, getTypeSymbol, ReactPriorityLevels, ReactTypeOfWork, ReactTypeOfSideEffect, } = getInternalReactConstants(version);
    const { PerformedWork } = ReactTypeOfSideEffect;
    const { ClassComponent, DehydratedSuspenseComponent, ForwardRef, Fragment, FunctionComponent, HostRoot, HostPortal, HostComponent, HostText, IncompleteClassComponent, IndeterminateComponent, LegacyHiddenComponent, MemoComponent, OffscreenComponent, Profiler, SimpleMemoComponent, SuspenseComponent, SuspenseListComponent, ContextConsumer, LazyComponent, } = ReactTypeOfWork;
    ...
    const idToArbitraryFiber = new Map();
    // Map for fiber type
    let typeIdSeed = 1;
    const fiberTypeId = new WeakMap();
    const fiberTypeIdNonWeakRef = new Map();
    ...
    function removeFiber(fiber, refs) {
        ...
        if (typeof id !== "number") {
            console.error("[React Render Tracker] removeFiber can't resolve an id by a given fiber");
            return;
        }
        ...
        if (stateNode) {
            trackObjectForLeaking(stateNode, id, TrackingObjectStateNode, displayName, null);
        }
        if (alternate) {
            fiberToId.delete(alternate);
            trackObjectForLeaking(alternate, id, TrackingObjectAlternate, displayName, null);
        }
        if (memoizedState && "next" in memoizedState) {
            let cursor = memoizedState;
            while (cursor !== null) {
                if (typeof cursor.queue?.dispatch === "function") {
                    trackObjectForLeaking(cursor.queue.dispatch, id, TrackingObjectHook, displayName, cursor.queue.dispatch.hookIdx);
                }
                else if (typeof cursor.memoizedState?.create === "function") {
                    trackObjectForLeaking(cursor.memoizedState.create, id, TrackingObjectHook, displayName, cursor.memoizedState.create.hookIdx);
                }
                cursor = cursor.next;
            }
        }
