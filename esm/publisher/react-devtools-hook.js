...
export function createReactDevtoolsHook(attachRenderer, onUnsupportedRenderer, existing) {
    const attachedIntegrations = new Map();
    ...
    const reactDevtoolsHook = {
        ...
        inject(renderer, rendererId) {
            ...
            if (unsupportedRender) {
                console.warn(`[react-render-tracker] ${unsupportedRender.reason}`, unsupportedRender.info);
                ...
                if (attachedIntegrations.size === 0) {
                    ...
                    console.warn(`[react-render-tracker] Only one React instance per page is supported for now, but one more React instance (${renderer.rendererPackageName} v${renderer.version}) was detected`);
                }
            }
            return id;
        },
...
const MARKER = Symbol();
export function installReactDevtoolsHook(target, attachRenderer, onUnsupportedRenderer) {
