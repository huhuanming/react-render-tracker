...
export default function WaitingForReady({ children, }) {
    const fiberRoots = useFiberChildren(0);
    const { loadedEventsCount, totalEventsCount } = useEventsContext();
    if (fiberRoots.length > 0) {
        return children;
    }
    return (React.createElement("div", { className: "waiting-for-ready" }, totalEventsCount > 0
        ? loadedEventsCount === totalEventsCount
            ? "Rendering..."
            : `Loading events (${Math.trunc((100 * loadedEventsCount) / totalEventsCount)}%)...`
