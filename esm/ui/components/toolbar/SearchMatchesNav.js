...
const SearchMatchesNav = ({ groupByParent, showUnmounted, autoselect, pattern, }) => {
    ...
    const selectDebounced = React.useMemo(() => debounce(id => select(id, false), 100, {
        maxWait: 200,
    }), []);
    React.useEffect(() => {
    ...
    const disableButtons = matches.total === 0 || (matches.total === 1 && matches.index === 1);
    return (React.createElement("div", { className: "component-search-matches-nav" },
        React.createElement("span", { className: "component-search-matches-nav__label" }, matches.total
            ? `${matches.index || "–"} / ${matches.total}`
            : "No matches"),
        React.createElement("span", { className: "component-search-matches-nav__buttons" },
            React.createElement("button", { className: "component-search-matches-nav__button", disabled: disableButtons, onClick: () => {
                    const startNode = treeRoot === tree.root
                        ? selectedNode
                        : selectedNode !== null && treeRoot.contains(selectedNode)
                            ? selectedNode
                            : null;
                    const node = treeRoot.findBack(node => node.id !== selectedId &&
                        match(node.fiber?.displayName || null) !== null, startNode);
                    if (node) {
            ...
            React.createElement("button", { className: "component-search-matches-nav__button", disabled: disableButtons, onClick: () => {
                    const startNode = treeRoot === tree.root
                        ...
                        : selectedNode !== null && treeRoot.contains(selectedNode)
                            ? selectedNode
                            : null;
                    const node = treeRoot.find(node => node.id !== selectedId &&
                        match(node.fiber?.displayName || null) !== null, startNode);
                    if (node) {
                        select(node.id, false);
                    }
