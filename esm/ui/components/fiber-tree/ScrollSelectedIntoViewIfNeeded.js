...
export const ScrollFiberIntoViewIfNeeded = () => {
    ...
    React.useEffect(() => {
        ...
        const element = id !== null ? getFiberElement(id) || null : null;
        if (element !== null) {
            const viewportEl = getOverflowParent(element);
            const elementRect = getBoundingRect(element, viewportEl);
            const scrollMarginTop = parseInt(getComputedStyle(element).getPropertyValue("scroll-margin-top"), 10) || 0;
            const scrollMarginLeft = parseInt(getComputedStyle(element).getPropertyValue("scroll-margin-left"), 10) || 0;
