...
export function useComputeSubscription(compute, subscribe) {
    ...
    useSubscription(() => subscribe(() => {
        ...
        syncStateValue();
    }, acceptRecomputeRequestsRef.current), [subscribe]);
...
export function useDebouncedComputeSubscription(compute, subscribe, debounced) {
    const subscribeWithDebouncedCompute = React.useCallback((requestRecompute, acceptRequests) => {
